...
class AQIDataFetcher:
    ...
    def fetch_data(self):
        ...
        try:
            ...
            # IMPROVED LOCATION DETECTION (Reverse Geocoding)
            # Try to get a more specific name (e.g., Village/area) using Geopy
            try:
                from geopy.geocoders import Nominatim
                geolocator = Nominatim(user_agent="weather_app_ml_project")
                # Use the requested lat/lon from the object (which comes from user), not the station lat/lon
                # We can fallback to station lat/lon if needed, but user location is priority
                target_lat, target_lon = 0, 0
                
                # Check if we were initialized with specific coords
                if hasattr(self, 'base_url') and 'geo:' in self.base_url:
                    # Extract from url "feed/geo:lat;lon/"
                    parts = self.base_url.split('geo:')[1].split('/')[0].split(';')
                    target_lat, target_lon = float(parts[0]), float(parts[1])
                else:
                    target_lat, target_lon = metadata['lat'], metadata['lon']

                location = geolocator.reverse((target_lat, target_lon), language='en', zoom=14)
                if location:
                    ...
                    broad_name = addr.get('city') or addr.get('county') or addr.get('state_district')
                    
                    if specific_name:
                         # e.g., "Kolavda, Gandhinagar"
                         metadata['city'] = f"{specific_name}, {broad_name}" if broad_name else specific_name
                    elif broad_name:
                         metadata['city'] = broad_name
                    
                    print(f"Reverse Geocoded Location: {metadata['city']}")
...
class AQIModels:
    ...
    def generate_forecast(self, model, df, metadata, disaster_predictor, weather_fetcher=None, days=7):
        ...
        # FIX: Force forecast to start from TOMORROW, regardless of where history ends
        today = datetime.now().date()
        future_dates = [pd.Timestamp(today + timedelta(days=i)) for i in range(1, days + 1)]
        forecast_values = []
